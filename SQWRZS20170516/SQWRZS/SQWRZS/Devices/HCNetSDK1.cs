...
namespace SQWRZS
{
    class HCNetSDK1
    {
        ...
        **************************************************/
        ...
        public const int COMM_UPLOAD_PLATE_RESULT = 0x2800; //车牌自动识别信息上传
        ...
        **********************************************************/
        ...
        /// 注册回调函数，接收设备报警消息
        /// </summary>
        /// <param name="fMessageCallBack">报警信息回调函数</param>
        /// <param name="pUser">用户数据</param>
        /// <returns></returns>
        [DllImport("HCNetSDK.dll")]
        ...
        **********************************************************/
        ...
        /// 建立报警上传通道，获取报警等信息
        /// </summary>
        /// <param name="lUserID">NET_DVR_Login_V30 的返回值</param>
        /// <returns>-1 表示失败，其他值作为 NET_DVR_CloseAlarmChan_V30 函数的句柄参数。</returns>
        [DllImport("HCNetSDK.dll")]
        public static extern int NET_DVR_SetupAlarmChan_V30(int lUserID);
        /// <summary>
        /// 撤销报警上传通道
        /// </summary>
        /// <param name="lAlarmHandle">NET_DVR_SetupAlarmChan_V30 的返回值</param>
        ...
        */
        ...
        public struct NET_DVR_PLATE_INFO
        {
            ...
            public char[] sLicense;  //车牌号码
            [MarshalAsAttribute(UnmanagedType.ByValArray, SizeConst = MAX_LICENSE_LEN, ArraySubType = UnmanagedType.I1)]
            public byte[] byBelieve; //各个识别字符的置信度，如检测到车牌"浙A12345", 置信度为20,30,40,50,60,70，则表示"浙"字正确的可能性是20%，"A"字的正确的可能性是30% 
        }

        [StructLayoutAttribute(LayoutKind.Sequential, CharSet = CharSet.Ansi)]
        public struct NET_DVR_VEHICLE_INFO
        {
            public UInt32 dwIndex;   //车辆序号 
            public byte byVehicleType;  //车辆类型，0-其他车辆，1-小型车，2-大型车
            public byte byColorDepth;  //车身颜色深浅，0-深色，1-浅色
            public byte byColor;  //车身颜色，0-其他色，1-白色，2-银色，3-灰色，4-黑色，5-红色，6-深蓝，7-蓝色，8-黄色，9-绿色，10-棕色，11-粉色，12-紫色
            public byte byRes1;//保留
            public UInt16 wSpeed; //车辆速度，单位km/h
            public UInt16 wLength;  //车身长度
            public byte byIllegalType;  //0-正常：1-低速，2-超速，3-逆行，4-闯红灯
            [MarshalAsAttribute(UnmanagedType.ByValArray, SizeConst = 35, ArraySubType = UnmanagedType.I1)]
            public byte[] byRes;  //保留 
        }
        ...
        //车牌检测结果
        /// <summary>
        /// 智能高清IPC报警上传的信息是图片或者录像中的一种，可以通过判断图片和录像的长度是否为0以确定上传的是图片信息还是录像信息。
        /// 图片数据为场景图片+车牌小图片。视频长度为0xffffffff时，表示视频内容异常，此时只上传报警信息，后面无视频内容，
        /// 指向视频内容的指针为NULL。iDS-65xx手动抓拍只支持上传pBuffer1和pBuffer2。
        /// /// </summary>
        [StructLayoutAttribute(LayoutKind.Sequential)]
        public struct NET_VCA_PLATE_RESULT
        {
            public UInt32 dwSize;  //结构体大小 
            public byte byResultType;  //识别结果类型，0-通过视频识别，1-通过图像识别
            public byte byChanIndex; //车道号 
            [MarshalAsAttribute(UnmanagedType.ByValArray, SizeConst = 2, ArraySubType = UnmanagedType.I1)]
            public byte[] byRes1;  //保留
            public UInt32 dwRelativeTime; //相对时间（保留）
            [MarshalAsAttribute(UnmanagedType.ByValArray, SizeConst = 32, ArraySubType = UnmanagedType.I1)]
            public byte[] byAbsTime;  //绝对时间，精确到毫秒，yyyymmddhhmmssxxx，例如20090810235959999 
            public UInt32 dwPicLen;  //图片长度（近景图）
            public UInt32 dwPicPlateLen;  //车牌小图片长度（车牌彩图）
            public UInt32 dwVideoLen;  //录像内容长度 
            public byte byTrafficLight;  //0-非红绿灯抓拍，1-绿灯时抓拍；2-红灯时抓拍
            public byte byPicNum;  //连拍的图片序号
            public byte byDriveChan;  //触发的车道号 
            public byte byRes2;  //保留
            public UInt32 dwBinPicLen;  //二值图长度（仅iDS-65xx支持）
            public UInt32 dwCarPicLen;  //车辆原图长度（仅iDS-65xx支持）
            public UInt32 dwFarCarPicLen;  //远景图长度（仅iDS-65xx支持）
            public IntPtr pBuffer3;  //车牌二值图（仅iDS-65xx支持
            public IntPtr pBuffer4;//车辆原图（仅iDS-65xx支持）
            public IntPtr pBuffer5;  //远景图（仅iDS-65xx支持） 
            [MarshalAsAttribute(UnmanagedType.ByValArray, SizeConst = 8, ArraySubType = UnmanagedType.I1)]
            public byte[] byRes3; //保留
            public NET_DVR_PLATE_INFO struPlateInfo;  //车牌信息参数 
            public NET_DVR_VEHICLE_INFO struVehicleInfo;  //车辆信息参数
            public IntPtr pBuffer1;  //当上传的是图片(近景图)信息时，指针指向图片信息，图片长度为dwPicLen；当上传的是录像时，指针指向录像信息，录像长度为dwVideoLen 
            public IntPtr pBuffer2; //当上传的是图片(车牌图)信息时，指针指向车牌小图片信息，车牌小图片的长度为dwPicPlateLen 
        }
        ...
        ******************************************************************************/
        ...
        {
            /// <summary>
            /// 抓拍图片上是否关闭OSD信息叠加：0- 不关闭(默认)，1- 关闭 
            /// </summary>
        ...
        public struct NET_DVR_PLATE_RESULT
        {
            ...
            /// <summary>
            /// 绝对时间，精确到毫秒yyyymmddhhmmssxxx，例如20090810235959999 
            /// </summary>
            [MarshalAsAttribute(UnmanagedType.ByValArray, SizeConst = 32, ArraySubType = UnmanagedType.I1)]
        ...
        /// <returns></returns>
        [DllImportAttribute(@"HCNetSDK.dll")]
        public static extern bool NET_DVR_ManualSnap(int lUserID,
            ref NET_DVR_MANUALSNAP lpInter, ref NET_DVR_PLATE_RESULT lpOuter);

        /// <summary>
        /// 网络触发抓拍
        /// </summary>
        /// <param name="m_lUserID">注册设备ID</param>
        /// <param name="lpInter">网络触发抓拍配置</param>
        /// <returns></returns>
        [DllImportAttribute(@"HCNetSDK.dll")]
        public static extern bool NET_DVR_ContinuousShoot(int lUserID,
           ref NET_DVR_SNAPCFG lpInter);
